...
public class EntityUnitBrowser_Ownership_OrgReorg_Test extends BrowserInvoke {
	@BeforeSuite
	...
	public void Ownership_OrgReorg() throws InterruptedException, AWTException {
		...
		// Step----------Navigateto Ownership tab to OrgReorgtab--------------------------//
			lp.fnEntitiesTabNavigation("Ownership", "Org-Reorg");
				...
				for (int i = 2; i <= rows.size(); i++) {
					fm.fnWebTable(driver,
							driver.findElement(By
									.xpath("//DIV[@id='gridOrgReorg_grdEntityManager_dom']/TABLE[1]/TBODY[1]/TR[" + i + "]")),
							"Click");
				}
		...
		// Step-----------Delete---------------------------------//
			...
			if (rows1.size() >= 2) {
				...
				for (int i = 2; i <= rows1.size(); i++) {
					fm.fnWebTable(driver,driver.findElement(By.xpath("//DIV[@id='gridOrgReorg_grdEntityManager_dom']/TABLE[1]/TBODY[1]/TR[" + i + "]")),"Click");
				}
				robot.keyRelease(KeyEvent.VK_CONTROL);
				fm.fnWebButton(driver, By.xpath("//img[@id='btnActionsMenu']"),"Actions");
				lp.fnOWMActionsMenu("Delete", "");
