...
public class OWM extends ExtentManager{
	...
	public void Actions() throws InterruptedException {
		...
		wait1.until(ExpectedConditions.visibilityOfElementLocated(Actions));
	...
	public void fnOWMActionsMenu(String item, String subItem) throws InterruptedException {
		...
		for (int i = 0; i < menuitems.size(); i++) {
			...
			if (item.equals(menuitems.get(i).getText())) {
				...
				System.out.println(driver.findElement(By.id(menuitem)).getText());
	...
	public void fnNavigateTab(String tab) throws InterruptedException {
		...
		if (driver.getTitle().equalsIgnoreCase("WorkFlow Manager")) {
			driver.manage().window().maximize();
			wait1.until(ExpectedConditions.frameToBeAvailableAndSwitchToIt("bottom"));
			wait1.until(ExpectedConditions.frameToBeAvailableAndSwitchToIt("content"));
			wait1.until(ExpectedConditions.frameToBeAvailableAndSwitchToIt("bottomFrame"));
	...
	public void fnNewFolderCreation() throws InterruptedException {
		...
		/*
		 * Set<String> ids1 = driver.getWindowHandles(); java.util.Iterator<String> it1
		 * = ids1.iterator(); String parentid = it1.next(); String childid = it1.next();
		 * String childid1 = it1.next();
		 */
		FluentWait<WebDriver> wait1 = new FluentWait<WebDriver>(driver).withTimeout(Duration.ofSeconds(70))
				.pollingEvery(Duration.ofSeconds(5)).ignoring(NoSuchElementException.class, NoSuchWindowException.class)
				.ignoring(NoSuchFrameException.class);
		try {
			loginPage lp = new loginPage(driver, template, template);
			//Boolean temp = fnVisibilityTest(nf_Year);
			//if (temp.TRUE) {
				// if (driver.getTitle().contains("New Folder")) {
				wait1.until(ExpectedConditions.visibilityOfElementLocated(nf_Year));
			...
			if (isAlertPresent(driver)) {
				if (driver.switchTo().alert().getText().equalsIgnoreCase("A Folder already exists")) {
					...
					Thread.sleep(4000);
					wait1.until(ExpectedConditions.frameToBeAvailableAndSwitchToIt("viewIFrame"));
	...
	public void fnOWMNewWorkFlow() throws InterruptedException {
		...
		FluentWait<WebDriver> wait1 = new FluentWait<WebDriver>(driver).withTimeout(Duration.ofSeconds(50))
				...
				.ignoring(NoSuchFrameException.class);
		try {
			wait1.until(ExpectedConditions.visibilityOfElementLocated(WFP_NWF_WFTemplate));
	...
	public void fnOWMCustomizeView(String[] array) throws InterruptedException {
		...
		if (driver.getTitle().equalsIgnoreCase("Grid Columns")) {
			...
			wait.until(ExpectedConditions.visibilityOfElementLocated(CustView_Save));
	...
	public void fnOWMSavePreferences(String menuitem) throws InterruptedException {
		...
		if (menuitem.equals("Save Preferences for All")) {
			if (driver.getTitle().equals("Save Preferences")) {
				...
				wait1.until(ExpectedConditions.visibilityOfElementLocated(SavePreforAll_notify));
				fm.fnWebCheckBox(driver, SavePreforAll_notify, "Notify by Email");
				Thread.sleep(500);
				wait1.until(ExpectedConditions.visibilityOfElementLocated(SavePreforAll_save));
	...
	public void fnWorkflowProperties() {
		childTest = test.createNode(
				"Description: Verifying Workflow Properties" + "<br>" + "<< Screen Name: Workflow Properties Webpage >></br>");
		FluentWait<WebDriver> wait1 = new FluentWait<WebDriver>(driver).withTimeout(Duration.ofSeconds(50))
				.pollingEvery(Duration.ofSeconds(5)).ignoring(NoSuchElementException.class, NoSuchWindowException.class)
				.ignoring(NoSuchFrameException.class);
		try {
			loginPage lp = new loginPage(driver, template, template);
			wait1.until(ExpectedConditions.visibilityOfElementLocated(nf_WFTemplate));
			fm.fnWebEditCompare(driver, nf_WFTemplate, template.getProperty("WF_Template"), "WorkFlow Template");
			fm.fnWebEditCompare(driver, nf_EntityName, template.getProperty("Entity_Name"), "EntityName");
			fm.fnWebEditCompare(driver, nf_EntityID, template.getProperty("Entity_Id"), "Entity ID");
			fm.fnWebEditCompare(driver, nf_Jurisdiction, template.getProperty("Jurisdiction"), "Jurisdiction");	
			fm.fnWebButton(driver, nf_SaveOK, "Save");
		} catch (Exception e) {
			childTest.fail(e);
		}
